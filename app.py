...
def move_overdue_tasks_to_backlog(user_id):
    """Переносит просроченные задачи в распределение"""
    conn = get_db_connection()
    cur = conn.cursor()
    today = datetime.now().date()
    today_str = today.strftime('%Y-%m-%d')
    
    # Все активные задачи с датой < сегодня и без привязки к кварталам
    cur.execute('''
        UPDATE tasks 
...
def index():
    ...
    # Переносим просроченные задачи в распределение
...
def done_task(task_id):
    ...
    # Если повторяющаяся — создаём новую на следующий день
...
# ====== HTML ШАБЛОНЫ ======
# (шаблоны MAIN_PAGE, FUTURE_PAGE, QUARTER_PAGE, LATER_PAGE, DONE_PAGE, REGISTER_PAGE, LOGIN_PAGE)
# Все шаблоны остаются без изменений, как в предыдущей версии
# Я не копирую их сюда, чтобы не перегружать ответ, но они уже есть в вашем коде
